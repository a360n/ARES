...
// 🕹️ معالج أوامر الحركة (Forwarding commands to Pico)
static esp_err_t move_handler(httpd_req_t *req) {
  ...
  // إرسال الحركة عبر السيريال للبيكو
  Serial.print(dir);
  Serial2.printf("[USB LOG] Received command from MacBook: '%s' -> Forwarding to Pico on GP1\n", dir);
  log_i("Forwarded movement command: %s", dir);

  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "OK", 2);
}

// 🎥 معالج حركة السيرفو والتحكم بزوايا الكاميرا مباشرة من الكاميرا
#define PAN_PIN 13
#define TILT_PIN 15
...
void writeServoAngle(int pin, int channel, int angle) {
  ...
  // تحويل الزاوية 0-180 إلى عرض نبضة 500us - 2500us (تردد 50Hz يعني دورة 20ms)
  double pulseWidth = 0.5 + ((double)angle / 180.0) * 2.0;
  uint32_t duty = (uint32_t)((pulseWidth / 20.0) * 4095.0); // دقة 12 بت (0-4095)
...
static esp_err_t servo_handler(httpd_req_t *req) {
  ...
  if (httpd_query_key_value(buf, "pan", pan_str, sizeof(pan_str)) == ESP_OK) {
    pan_val = atoi(pan_str);
    writeServoAngle(PAN_PIN, 2, pan_val);
  }
  if (httpd_query_key_value(buf, "tilt", tilt_str, sizeof(tilt_str)) == ESP_OK) {
    tilt_val = atoi(tilt_str);
    if (tilt_val > 90) tilt_val = 90;
    if (tilt_val < 0) tilt_val = 0;
    writeServoAngle(TILT_PIN, 3, tilt_val);
  }
  
  free(buf);

  Serial2.printf("[USB LOG] Servo command received: Pan=%d, Tilt=%d\n", pan_val, tilt_val);

  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "OK", 2);
}

// 📡 معالج التليمتري والبيانات البيئية المرسلة من البيكو
static esp_err_t telemetry_handler(httpd_req_t *req) {
  String localCopy;
  {
    std::lock_guard<std::mutex> lock(telemetryMutex);
    localCopy = latestTelemetry;
  }

  // تنسيق المتغير: DATA:temp,hum,mq9,mq135,distance,lat,lng,encR,encL,battery
  if (localCopy.startsWith("DATA:")) {
    localCopy = localCopy.substring(5);
  }

  String temp = "0", hum = "0", mq9 = "0", mq135 = "0", distance = "0";
  String lat = "No Fix", lng = "No Fix", encR = "0", encL = "0", battery = "0";

  int idx = 0;
  int startPos = 0;
  for (int i = 0; i <= localCopy.length(); i++) {
    if (i == localCopy.length() || localCopy.charAt(i) == ',') {
      String val = localCopy.substring(startPos, i);
      val.trim();
      switch (idx) {
        case 0: temp = val; break;
        case 1: hum = val; break;
        case 2: mq9 = val; break;
        case 3: mq135 = val; break;
        case 4: distance = val; break;
        case 5: lat = val; break;
        case 6: lng = val; break;
        case 7: encR = val; break;
        case 8: encL = val; break;
        case 9: battery = val; break;
      }
      idx++;
      startPos = i + 1;
    }
  ...
  snprintf(json, sizeof(json),
    "{\"temp\":\"%s\",\"hum\":\"%s\",\"mq9\":\"%s\",\"mq135\":\"%s\",\"distance\":\"%s\","
    "\"lat\":\"%s\",\"lng\":\"%s\",\"encR\":\"%s\",\"encL\":\"%s\",\"battery\":\"%s\"}",
    temp.c_str(), hum.c_str(), mq9.c_str(), mq135.c_str(), distance.c_str(),
    lat.c_str(), lng.c_str(), encR.c_str(), encL.c_str(), battery.c_str()
  );
...
void setupServos() {
#if ESP_ARDUINO_VERSION_MAJOR >= 3
  ledcAttach(PAN_PIN, 50, 12);
  ledcAttach(TILT_PIN, 50, 12);
#else
  ledcSetup(2, 50, 12);
  ledcAttachPin(PAN_PIN, 2);
  ledcSetup(3, 50, 12);
  ledcAttachPin(TILT_PIN, 3);
#endif

  // توسيط المحور الأفقي على 90 درجة والعمودي على 30 درجة عند التشغيل
  writeServoAngle(PAN_PIN, 2, 90);
  writeServoAngle(TILT_PIN, 3, 30);
}
